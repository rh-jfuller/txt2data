...
fn validate(grammar: &Grammar) -> Result<(), GrammarError> {
    ...
    // Check nonterminal references are defined.
    // Skip synthetic names (prefixed with __) generated by
    // grammar parser for desugared repetition operators.
...
struct GrammarParser {
    chars: Vec<char>,
    pos: usize,
    /// Synthetic rules generated by groups
...
impl GrammarParser {
    ...
    fn parse(&mut self) -> Result<Grammar, GrammarError> {
        ...
        rules.append(&mut self.pending_rules);
        Ok(Grammar { rules })
    }
    ...
    fn parse_group(&mut self) -> Result<Term, GrammarError> {
        ...
        if alts.len() == 1 && alts[0].len() == 1 {
            ...
            let name = format!("__group_{}", self.pos);
    ...
    fn try_parse_repeat(&mut self, term: Term) -> Result<Vec<Term>, GrammarError> {
        ...
        match self.peek() {
            Some('*') => {
                self.advance();
                let sep = self.try_parse_separator()?;
                self.skip_ws();
                if let Some(sep) = sep {
                    Ok(self.make_star_sep(term, sep))
                } else {
                    Ok(vec![
                        term,
                        Term::Nonterminal {
            ...
            Some('+') => {
                ...
                self.skip_ws();
                if let Some(sep) = sep {
                    Ok(self.make_plus_sep(term, sep))
                } else {
                    Ok(vec![
                        term,
                        Term::Nonterminal {
    ...
    fn make_star_sep(&mut self, term: Term, sep: Term) -> Vec<Term> {
        let tail_name = format!("__septail_{}", self.pos);
        self.pending_rules.push(Rule {
            mark: Mark::Hidden,
    ...
    fn make_plus_sep(&mut self, term: Term, sep: Term) -> Vec<Term> {
        let tail_name = format!("__septail_{}", self.pos);
        self.pending_rules.push(Rule {
            mark: Mark::Hidden,
            name: tail_name.clone(),
    ...
    fn parse_name(&mut self) -> Result<String, GrammarError> {
        ...
        while let Some(c) = self.peek() {
            if c == '.' {
                if let Some(next) = self.peek_at(self.pos + 1)
                    && (is_name_follow(next) || is_name_start(next))
                {
                    name.push(c);
                    self.advance();
                    continue;
    ...
    fn try_consume_str(&mut self, s: &str) -> bool {
        ...
        // Ensure next char is not a name follower
